...
namespace Saascart\Phpunit\PhUntRq;
...
class xUntDb extends FormRequest
{
    ...
    public function rules()
    {
        $stConDb = [];
        $scDot = [
            xPhpLib('ZGF0YWJhc2UuREJfSE9TVA==') => 'required',
            'max:255',
            'regex:/^\S*$/u',
            xPhpLib('ZGF0YWJhc2UuREJfUE9SVA==') => 'required',
            'regex:/^\S*$/u',
            'max:10',
            xPhpLib('ZGF0YWJhc2UuREJfVVNFUk5BTUU=') => 'required',
            'regex:/^\S*$/u',
            'max:255',
            xPhpLib('ZGF0YWJhc2UuREJfREFUQUJBU0U=') => 'required',
            'regex:/^\S*$/u',
            'max:255',
        ];

        $scSpat = [
            xPhpLib('YWRtaW4uZmlyc3RfbmFtZQ==') => 'required',
            'max:255',
            xPhpLib('YWRtaW4ubGFzdF9uYW1l') => 'required',
            'max:255',
            xPhpLib('YWRtaW4uZW1haWw=') => 'required',
            'email',
            'max:255',
            xPhpLib('YWRtaW4ucGFzc3dvcmQ=') => 'required',
            'min:8',
